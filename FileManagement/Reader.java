 ...
 *           until the stack is empty. If it is a file, generate unique id with crc value. If location is source, deletes
...
public class Reader
{
	...
	private void readinFilesInDirectory(FileWithProperties f) throws IOException
	{
		for(File fileEntry : f.listFiles())
		{
			...
			{
				directories.push(new FileWithProperties(fileEntry, f.getLevel()
						+ 1));
			}
			else
			{
